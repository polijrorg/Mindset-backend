...
export default class CreateAppointmentService {
  ...
  public async execute({
    ...
    try {
      ...
      const patientCepCorrected = `${patientCep.toString().slice(0, 5)}-${patientCep.toString().slice(5)}`;
      const establishmentCep = `${establishment.cep.toString().slice(0, 5)}-${establishment.cep.toString().slice(5)}`;
      const patientLocationResponse = await axios.get(`https://cdn.apicep.com/file/apicep/${patientCepCorrected}.json`);
      ...
      const origin = `
      ${originAddress.replaceAll(' ', '%20').replaceAll(',', '%2C')}
      ${originDistrict.replaceAll(' ', '%20').replaceAll(',', '%2C')}
      ${originCity.replaceAll(' ', '%20').replaceAll(',', '%2C')}
      ${originState.replaceAll(' ', '%20').replaceAll(',', '%2C')}
    `;
      ...
      const destination = `
      ${destinyAddress.replaceAll(' ', '%20').replace(',', '%2C')}
      ${destinyDistrict.replaceAll(' ', '%20').replace(',', '%2C')}
      ${destinyCity.replaceAll(' ', '%20').replaceAll(',', '%2C')}
      ${destinyState.replaceAll(' ', '%20').replaceAll(',', '%2C')}
    `;
      ...
      if (distanceArrayResponse.data.rows[0].elements[0].status === 'NOT_FOUND') { return {} as Appointment; }
      console.log(distanceArrayResponse.data.rows[0].elements[0]);
